...
![Simple 2D Triangle Vertices](Documentation/1-example-triangle.png)
...
![Main Stages of the Metal Graphics Render Pipeline](Documentation/2-render-pipeline.png)

Rendering starts with a drawing command, which includes a vertex count and what kind of primitive to render.
For example, here's the drawing command from this sample:
...
Decide what data to pass into your render pipeline and what data is passed to later stages of the pipeline.
There are typically three places where you do this:
...
The output position (described in detail below) must be defined as a `vector_float4`.
The color is declared as it was in the input data structure.
...
Normalized device coordinates are independent of viewport size.

![Normalized device coordinate system](Documentation/3-normalized-coordinates.png)
...
![Vertex function coordinate transformation](Documentation/4-coordinate-transformation.png)
...
![Fragments generated by the rasterization stage](Documentation/5-fragment-function.png)
...
- Note: The reason a fragment is called a possible change is because the pipeline stages after the fragment stage can be configured to reject some fragments or change what gets written to the render targets.
...
![Interpolated Fragment Colors](Documentation/6-rasterization-stage.png)
...
First, get the default library and obtain an [`MTLFunction`][MTLFunction] object for each function.
...
Next, create an [`MTLRenderPipelineState`][MTLRenderPipelineState] object.
Render pipelines have more stages to configure, so you use an [`MTLRenderPipelineDescriptor`][MTLRenderPipelineDescriptor] to configure the pipeline.
